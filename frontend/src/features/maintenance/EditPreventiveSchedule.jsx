...
const EditPreventiveSchedule = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [scheduleRes, assetsRes] = await Promise.all([
        api.get(`/maintenance/preventive/${id}/`),
        api.get('/assets/assets/')
      ])
      
      const schedule = scheduleRes.data
      setFormData({
        asset: schedule.asset.id,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      await api.patch(`/maintenance/preventive/${id}/`, formData)
      showSuccess('Preventive maintenance schedule updated successfully!')
      setTimeout(() => {
      ...
      if (err.response?.data) {
        ...
        const errorMessages = Object.entries(errors)
          ...
          .join('. ')
        showError(`Failed to update schedule: ${errorMessages}`)
      } else {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8 animate-slide-down">
        ...
        <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-2xl p-8 text-white shadow-xl">
          ...
          <p className="text-indigo-100 text-lg">Update the preventive maintenance schedule details</p>
        </div>
      </div>
...
export default EditPreventiveSchedule
